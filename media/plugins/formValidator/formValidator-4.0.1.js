document.write('<script language="javascript" src="/site_media/js/jquery.jbasicext.js"></script>');
//====================================================================================================
...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		var settings = 
		{
			...
			delaytime: 5000, //5秒
			reset: true,
			messageID: '#tipMessage',
			loadingImg: '/site_media/images/loading.gif'
		...
		//如果填写了表单和按钮，就注册验证事件
		if(settings.formID!=""){
			$("#"+settings.formID).submit(function(){return false;});
		}
		else if(settings.submitButtonID!="")
		{
			$("#"+settings.submitButtonID).click(function(){return $.formValidator.bindSubmit(settings);});
		}
		if(settings.ajaxSubmitID!="") {
			$("#"+settings.ajaxSubmitID).click(function(){
				if($.formValidator.bindSubmit(settings)){
					$.ajaxform.hookajax(settings.validatorGroup);
				}
			});
		}
...
//ajax异步提交数据
$.ajaxform = {
	hookajax: function(validatorGroup){
		var initConfig = $('body').data(validatorGroup);
		e = $("#"+initConfig.formID);
		var tg = $(initConfig.trigger);
		if(initConfig.trigger==""){
			tg = $("#"+initConfig.ajaxSubmitID);
		}
		
		var url = e.attr('action') || location.href;
		$.ajax({
			type: 'POST',
			url: url,
			data: $(e).getdict(),
			beforeSend: function(r){//alert($(e).getdict()[0].value)
				
				tg.attr('disabled', true).after(' <img src="$0" class="loading" align="absmiddle"/>'.template([initConfig.loadingImg]));
			},
			success: function(data){
				$.ajaxform.data_handle(initConfig, data);
			},
			error: function(r, err, d){
				$.ajaxform.disp_message(initConfig, err);
				tg.attr('disabled', false).next('img.loading').remove();
			},
			complete: function(){
				tg.attr('disabled', false).next('img.loading').remove();
			}
	...
	data_handle : function(e, data){
		...
		//$('.error', e).html('');
		$("#"+e.formID).find('.onError').removeClass('onError').html('')
		...
		if (r.response == 'ok'){ //success
			...
			if (!$.ajaxform.call_func(e, 'on_success_ajax', r)){
				//call ondata callback
				if(r.data){$.ajaxform.call_func(e, 'ondata', r.data)};
				//if there is a next hidden text, then redirect to next url
				var next = r.next;
				if(next){window.location = next}
				else{$.ajaxform.disp_message(e, r.message)};
				$.ajaxform.call_func(e, 'on_success_finish', r);
				if(e.reset) $("#"+e.formID).get(0).reset();//clear from data
			}
		}
		else{   //error
			
			if(!$.ajaxform.call_func(e, 'on_error_ajax', r))
			{
				for(var k in r.error){
					...
					if (v.constructor == Array) 
						err = ','.join(v);
					if (k == '_' || k=="__all__") 
						$.ajaxform.disp_message(e, err);
					else{
						var p = $("#"+k+"Tip");
						p.html(err);
						p.attr('class','onError').show();
					}
				}
				$.ajaxform.disp_message(e, r.message);
				$.ajaxform.call_func(e, 'on_error_finish', r);
			}
		}
		$.ajaxform.call_func(e, 'onfinish', r);
	},
	
	disp_message : function(e, msg){
		if(msg) $(e.messageID).setmessage(msg, e.delaytime);
	},
